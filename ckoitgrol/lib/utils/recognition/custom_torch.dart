...
class ModelObjectDetection {
  ...
  Widget renderBoxesOnImage(
    ...
    return LayoutBuilder(builder: (context, constraints) {
      ...
      Image imageWidget = Image.file(image);
      ...
      return FutureBuilder<Size>(
        ...
        builder: (context, snapshot) {
          ...
          double scale = min(
            constraints.maxWidth / snapshot.data!.width,
            constraints.maxHeight / snapshot.data!.height,
          );

          // Calculate actual dimensions after scaling
          double scaledWidth = snapshot.data!.width * scale;
          double scaledHeight = snapshot.data!.height * scale;

          return Stack(
            children: [
              Positioned(
                left: (constraints.maxWidth - scaledWidth) /
                    2, // Center horizontally
                top: (constraints.maxHeight - scaledHeight) /
                    2, // Center vertically
                width: scaledWidth,
                height: scaledHeight,
                child: Image.file(
              ...
              ...recognitions.map((re) {
                ...
                return Positioned(
                  left: (constraints.maxWidth - scaledWidth) / 2 +
                      (re.rect.left * scaledWidth),
                  top: (constraints.maxHeight - scaledHeight) / 2 +
                      (re.rect.top * scaledHeight) -
                      (20), // Scale the offset too
                  child: Column(
                    ...
                    children: [
                      Container(
                        height: 20, // Scale the height
                        alignment: Alignment.centerRight,
                        margin: EdgeInsets.only(left: 100 * scale),
                        color: usedColor,
                        child: Text(
                          "${re.className ?? re.classIndex.toString()}_${showPercentage ? "${(re.score * 100).toStringAsFixed(2)}%" : ""}",
                          style: TextStyle(fontSize: 12), // Scale the font size
                        ),
                      ),
                      Container(
                        width: re.rect.width * scaledWidth,
                        height: re.rect.height * scaledHeight,
                        decoration: BoxDecoration(
                          border: Border.all(
                            color: usedColor,
                            width: 3 * scale, // Scale the border width
                          ),
                          borderRadius: BorderRadius.all(
                            Radius.circular(
                                2 * scale), // Scale the border radius
                          ),
                        ),
                      ),
                    ],
